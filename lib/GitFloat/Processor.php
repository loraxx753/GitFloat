...
class Processor {
	...
	function __construct() {
		...
		$loader = new \Twig_Loader_Filesystem(ROOT_DIR.'/snippets');
	...
	public function run_commit_audit($auditSince, $branch, $commitRegex = false) {
		...
		return $this->twig->render('custom/commit_audit.twig', 
	...
	public function run_hotfix_audit() {
		...
		return $this->twig->render('custom/hotfix_audit.twig', array(
							'result' => $result));
	}
	...
	public function run_compare($compareCommitsFrom = 'dev', $compareCommitsTo = 'master') {
		...
		return $this->twig->render('custom/compare.twig',
							array('result' => $result,
	...
	private function parse_commit($commit) {
		...
		return $this->twig->render('base/mediaObject.twig', 
							array('image'  => $avatar, 
								  'heading' => $heading,
								  'content' => $content));
	}
